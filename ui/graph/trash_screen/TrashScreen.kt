package com.example.grqph.trash_screen
...
import com.example.grqph.navigation_drawer.Screens.TRASH_SCREEN
...
import com.example.grqph.top_action_bar.NoteTopAppBar
import com.example.grqph.top_action_bar.dialogs.EraseDialog
import java.io.File
                        ...
                        VerticalGrid(maxColumnWidth = 220.dp) {
                            observerTrashedNotes.value.filter { entity ->
                                ...
                                NoteCard(
                                    ...
                                    viewModule.deleteNote(Note(uid = it1.note.uid))
                                    it1.note.uid.let {
                                        File(ctx.filesDir.path + "/" + IMAGE_FILE, "$it.$JPEG").delete()
                                        File(ctx.filesDir.path + "/" + AUDIO_FILE, "$it.$MP3").delete()
                                    }
